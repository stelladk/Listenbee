...
import com.example.eventdeliverysystem.filehandler.FileHandler;
...
public class Broker {
    ...
    private void toCliConnection() {
        ...
        Thread task = new Thread(new Runnable() {
            ...
            public void run() {
                while (true) {
                    try {
                        final Socket connection = toCliServer.accept();
    ...
    private void notifyBrokers(final ArrayList<String> artists) {
        ...
        for (String br : brokersList) {
            final String broker = br;
